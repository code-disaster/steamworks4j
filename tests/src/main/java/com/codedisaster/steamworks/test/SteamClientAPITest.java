    ...
    protected void processInput(String input) throws SteamException {
        ...
        if (input.startsWith("stats global ")) {
            ...
            for (int i = 0; i < numFiles; i++) {
                ...
                System.out.println("# " + i + " : name=" + file + ", size=" + sizes[0] + ", exists=" + (exists ? "yes"
                        : "no"));
            }
            ...
            if (paths.length >= 3) {
                ...
                if (updateHandle != null) {
                    ...
                    remoteStorage.updatePublishedFileDescription(updateHandle, "Dummy UGC file *updated* by test " +
                            "application.");
                    remoteStorage.commitPublishedFileUpdate(updateHandle);
                }
            }
        } else if (input.equals("ugc query")) {
            SteamUGCQuery query = ugc.createQueryUserUGCRequest(user.getSteamID().getAccountID(),
